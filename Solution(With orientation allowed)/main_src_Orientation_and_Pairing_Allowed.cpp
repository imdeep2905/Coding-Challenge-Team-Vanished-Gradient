...
void take_input()
{
    ...
    */
    cout << "**********************************************************************************"<< endl;
    cout << "* Orientation and Pairing allowed."<< endl;
...
bool is_better_depth(vector<vector<ll>> cur, vector<vector<ll>> lcl_best, vector<vector<ll>> best,ll d, ll db, ll dt)
{
    ...
    */
    ...
    /*
    STRAT INFO
    Strategy 1: Width,Depth : Greater Value||Volume.
    Strategy 2: Width,Depth : Greater Value/Volume.
    Strategy 3: Width: Greater Value||Volume, Depth: Greater Value/Volume
    */
}
...
bool try_orientation(ll i, ll w, ll d)
{    
    /*
    Purpose   : This function analyze given two solutions and determine which is worth exploring furthur. 
    Arguments : i: index of switch for which we will try all orientation
                w: width of sub rack
                d: depth of sub rack
    return    : True or False.
    */
    sort(switches_cord[i].begin(), switches_cord[i].end()); 
    ...
    { 
        ...
        {
            poss.push_back({switches_cord[i][0], switches_cord[i][1], switches_cord[i][2]});
        }
    }while (next_permutation(switches_cord[i].begin(), switches_cord[i].end())); 
   
    if(poss.size() == 0)
    {
        return false;
    }
    ll index = 0, h = poss[0][0];
    for(ll j = 1; j < poss.size(); j ++)
    {
        if(poss[j][0] <= h)
            index = j;
    }
    for(ll j = 0 ; j < 3; j ++)
    {
        switches_cord[i][j] = poss[index][j];
    }
    return true;
}
...
vector< vector <ll> > fill_stride(ll x, ll y, ll d, ll w, vector < vector<ll> > prev_sol)
{
    ...
    */
    ...
    for(ll i = 0; i < knapsack_return[1].size(); i ++)
    {
        ...
        {
            //If pair of switches is selected
            //resolve their index and get original switches's index.
            pair <ll, ll> p =  resolve_pairing[switch_taken[knapsack_return[1][i]]]; 
            ll f = p.first;
            ll s = p.second;
            //Adding volume of both the switches
            ...
            volume +=   switches_cord[s][0] * 
                        switches_cord[s][1] * 
                        switches_cord[s][2];
            //Changing availability of both switches
            --available_switch[f];--available_switch[s];
            //storing type, orientation and coordinates similar to 
            //part in if but for both switches
